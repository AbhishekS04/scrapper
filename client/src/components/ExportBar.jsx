...
export default function ExportBar({ jobId, jobData }) {
  ...
  const totalLeaks = results.reduce((sum, r) => {
    const leaks = r.leakedData || r.leaked_data || {};
    return sum + (leaks.totalFindings || 0);
  }, 0);
      ...
      <div className="glass-panel px-3 sm:px-5 py-3 sm:py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="hidden md:flex items-center gap-3 text-[10px] font-mono">
            <span className="chip chip-green">{results.length} pages</span>
            {totalLeaks > 0 && <span className="chip chip-red">{totalLeaks} leaks</span>}
          </div>
        </div>
